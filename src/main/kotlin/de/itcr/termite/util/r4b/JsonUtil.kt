...
class JsonUtil {
    ...
    companion object {
        ...
        fun deserialize(string: String?, typeName: String): DataType? {
            ...
            else if (BackboneElementParser.supportsType(typeName)) backboneElementParser.parse(string, typeName) as BackboneElement
            else jsonParser.parseType(string, typeName)
        }

        inline fun <reified TYPE: DataType> deserialize(string: String?) =
            deserialize(string, TYPE::class.simpleQualifiedName()) as TYPE
        ...
        inline fun <reified TYPE: DataType> deserializeList(string: String?) =
            deserializeList(string, TYPE::class.simpleQualifiedName()) as List<TYPE?>
